...
if (!function_exists('homey_exp_affiliate_booking_link')) {
    function homey_exp_affiliate_booking_link()
    {
        global $post;

        $affiliate_booking_link = get_post_meta($post->ID, 'homey_exp_affiliate_link', true);
...
if (!function_exists('homey_loadmore_listings')) {
    function homey_loadmore_listings()
    {
		...
		//echo isset($the_query->request) ? $the_query->request : 'noep';
...
if (!function_exists('homey_copy_images_for_duplicated_post')) {
    function homey_copy_images_for_duplicated_post($listing_id = 0)
    {
        if (!empty($_POST['listing_image_ids']) && is_array($_POST['listing_image_ids'])) {
            ...
            foreach ($_POST['listing_image_ids'] as $img_id) {
                ...
                if ($img_id && $file = get_attached_file($img_id)) {
                    ...
                    if (@copy($file, $file2)) {
                        ...
                        $new_listing_image_ids[] = intval($img_id2);
            ...
            // featured image
            update_post_meta($listing_id, '_thumbnail_id', $new_listing_image_ids[0]);
