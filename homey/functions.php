...
define('HOMEY_THEME_VERSION', '2.3.5');
...
if ( ! wp_next_scheduled( 'homey_delete_spam_users' ) ) {
...
if (!function_exists('homey_delete_spam_users')) {
    function homey_delete_spam_users()
    {
        ...
        $ids = implode(",", $ids);
        if(!empty($ids)){
            $spam_user_ids = $wpdb->get_results("DELETE " . $prefix . "users WHERE ID in (" . $ids . ");");
            dd($spam_user_ids);
        }else{
...
if(isset($_GET['no-login-err'])){
    if(!function_exists('homey_no_ajax_login_modal_show')){
        function homey_no_ajax_login_modal_show() {
            if(!is_user_logged_in()){
                $error_msg = esc_html__('Check your credentials or contact to the admin.', 'homey');
